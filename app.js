(() => {
  ...
  const STORAGE_KEYS = {
    ...
    BUDGET_LINE_ID: 'quoteBuilder.budgetLineId',
  };
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    state.savedQuoteRefs.budgetLineId = storage.get(STORAGE_KEYS.BUDGET_LINE_ID) || '';
  ...
  function loadDefaultData() {
    ...
    state.savedQuoteRefs.budgetLineId = '';
    storage.remove(STORAGE_KEYS.EVENT_ID);
    storage.remove(STORAGE_KEYS.BUDGET_LINE_ID);
    workbookModel = null;
  ...
  function saveQuoteToBudget() {
    ...
    const guestsLabel = quote.guests > 0 ? `${quote.guests.toLocaleString()} pax` : 'Quote';
    ...
    let budgetLine = null;
    if (state.savedQuoteRefs.budgetLineId) {
      budgetLine = budgetLines.find((line) => line.id === state.savedQuoteRefs.budgetLineId) || null;
    }
    const quoteKey = buildQuoteKey({ ...quote, eventDateInput: eventDateIso });
    if (!budgetLine) {
      budgetLine = budgetLines.find((line) => line.quoteKey === quoteKey) || null;
    }
    if (!budgetLine) {
      budgetLine = { id: null };
    }
    ...
    budgetLine.cat = 'Quotation';
    budgetLine.item = `${venueName || 'Venue'} + ${packageName || 'Package'}`.trim();
    budgetLine.qty = 1;
    budgetLine.unit = Number(quote.total || 0);
    const existingTax = Number(budgetLine.tax);
    budgetLine.tax = Number.isFinite(existingTax) ? existingTax : 0;
    budgetLine.forecast = Number(quote.total || 0);
    const existingActual = Number(budgetLine.actual);
    budgetLine.actual = Number.isFinite(existingActual) ? existingActual : 0;
    if (!budgetLine.confirmed && (!budgetLine.notes || !budgetLine.notes.trim())) {
    ...
    state.savedQuoteRefs.budgetLineId = savedBudgetId;
    storage.set(STORAGE_KEYS.BUDGET_LINE_ID, savedBudgetId);
    ...
    announceStatus('Quote added to the budget. Confirm it once the client signs.', 'success');
  }
  ...
  function buildQuoteKey(quote) {
    ...
    return [venueKey, packageKey, guestKey, dateKey].join('|');
  }
})();
