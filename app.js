(() => {
  ...
  function parseWorkbookPackages(sheet, pricingMap, venues) {
    ...
    const packages = rows
      .map((row, index) => {
        ...
        const defaultGuests = toNumber(pickFirst(row, ['Guests', 'GuestCount', 'IncludedGuests', 'Included Guests']));
        const chargeType = parseChargeType(pickFirst(row, ['ChargeType', 'Billing', 'Per', 'Basis']), row.PerGuest);
        const basePrice = toNumber(pickFirst(row, ['Price', 'BasePrice', 'Rate', 'Amount']));
  ...
  function parseWorkbookAddons(sheet, pricingMap) {
    ...
    return rows
      .map((row, index) => {
        ...
        const basePrice = toNumber(pickFirst(row, ['Price', 'BasePrice', 'Rate', 'Amount']));
        const defaultQty = toNumber(pickFirst(row, ['Qty', 'Quantity', 'DefaultQty', 'Default Quantity']));
  ...
  function extractPricingFromRows(rows, defaultBasis) {
    ...
    rows.forEach((row) => {
      ...
      entries.get(key).tiers.push(tier);
    });
  ...
  function matchesKey(pkg, key) {
    ...
    return [pkg.id, pkg.name, pkg.group, pkg.venueId]
      .filter(Boolean)
