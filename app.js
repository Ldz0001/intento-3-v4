(() => {
    ...
    try {
      ...
      const headerRow = new docx.TableRow({
        ...
        children: ['Item', 'Details', 'Qty', 'Unit', 'Total'].map((text) =>
          new docx.TableCell({
            ...
            shading: applyShadeType({ fill: 'F4F1FF', color: 'FFFFFF' }, 'PERCENT_10'),
      ...
      const bodyRows = data.rows.map((row, index) => {
        const zebraShadeFactory = () => (index % 2 === 0
          ? applyShadeType({ fill: 'FFFFFF', color: 'FFFFFF' }, 'PERCENT_5')
          : applyShadeType({ fill: 'F9F5FF', color: 'FFFFFF' }, 'PERCENT_12'));
      ...
      const totalRow = new docx.TableRow({
        children: [
          new docx.TableCell({
            ...
            shading: applyShadeType({ fill: '8B5CF6', color: 'FFFFFF' }, 'PERCENT_25'),
  ...
  function svgTextToPngDataUrl(svgText) {
    ...
    const strategies = IS_SAFARI
      ? ['data-url', 'object-url', 'file-reader']
      : ['object-url', 'file-reader', 'data-url'];
