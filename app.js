(() => {
  ...
  function updateComparePricesTable(guestCount, eventDate, selectedVenueId, selectedPackageId, weekdayLabel) {
    const { compareTableBody, compareSummary } = state.refs || {};
    if (!compareTableBody) {
      return;
    }
    ...
    if (!comparisons.length) {
      ...
      cell.textContent = 'Enter a guest count to see pricing comparisons.';
      emptyRow.appendChild(cell);
    ...
    let visible = comparisons.slice(0, COMPARE_TABLE_LIMIT);
    let pinnedSelection = false;
    if (normalizedSelectedVenue && normalizedSelectedPackage) {
      const selectedIndex = comparisons.findIndex(
        (entry) => entry.venueKey === normalizedSelectedVenue && entry.packageKey === normalizedSelectedPackage
      );
      const alreadyVisible = visible.some(
        (entry) => entry.venueKey === normalizedSelectedVenue && entry.packageKey === normalizedSelectedPackage
      );
      if (selectedIndex >= 0 && !alreadyVisible) {
        visible = visible.slice(0, Math.max(visible.length - 1, 0));
        visible.push(comparisons[selectedIndex]);
        pinnedSelection = selectedIndex >= COMPARE_TABLE_LIMIT;
      }
    }
    ...
    visible.forEach((entry) => {
      ...
      if (entry.venueKey === normalizedSelectedVenue && entry.packageKey === normalizedSelectedPackage) {
        row.classList.add('is-active');
      }

      const rankCell = document.createElement('td');
      rankCell.className = 'compare-rank';
      rankCell.textContent = entry.rank.toString();
      if (entry.rank === 1) {
        const badge = document.createElement('span');
        badge.className = 'compare-badge';
        badge.textContent = 'Best value';
        badge.setAttribute('aria-label', 'Lowest total for current filters');
        rankCell.appendChild(badge);
      }
      row.appendChild(rankCell);

      const venueCell = document.createElement('td');
      venueCell.textContent = entry.venueName || '—';
      row.appendChild(venueCell);

      const packageCell = document.createElement('td');
      packageCell.textContent = entry.packageName || '—';
      row.appendChild(packageCell);

      const tierCell = document.createElement('td');
      const tierParts = [];
      if (entry.tierLabel) {
        tierParts.push(entry.tierLabel);
      } else if (entry.tierId) {
        tierParts.push(`Tier ${entry.tierId}`);
      }
      if (entry.guestsUsed) {
        tierParts.push(`${entry.guestsUsed.toLocaleString()} guests`);
      }
      tierCell.textContent = tierParts.length ? tierParts.join(' • ') : '—';
      row.appendChild(tierCell);

      const baseCell = document.createElement('td');
      baseCell.textContent = formatCurrency(entry.basePrice);
      row.appendChild(baseCell);

      const adjustmentsCell = document.createElement('td');
      adjustmentsCell.className = 'adjustments';
      const adjustments = [];
      if (Math.abs(entry.percentAdjustment) > 0.01) {
        const percentLabel = entry.percentAdjustment < 0 ? 'Weekend' : 'Percent';
        const descriptor = weekdayLabel ? `${percentLabel} (${weekdayLabel})` : percentLabel;
        adjustments.push(`${descriptor}: ${formatCurrency(entry.percentAdjustment)}`);
      }
      if (Math.abs(entry.flatAdjustment) > 0.01) {
        const descriptor = weekdayLabel ? `Flat (${weekdayLabel})` : 'Flat';
        adjustments.push(`${descriptor}: ${formatCurrency(entry.flatAdjustment)}`);
      }
      adjustmentsCell.textContent = adjustments.length ? adjustments.join(' • ') : '—';
      row.appendChild(adjustmentsCell);

      const totalCell = document.createElement('td');
      totalCell.textContent = formatCurrency(entry.total);
    ...
    if (compareSummary) {
      ...
      const primaryGuests = guestCount > 0
        ? guestCount
        : comparisons.find((entry) => entry.guestsUsed > 0)?.guestsUsed || 0;
      const guestLabel = primaryGuests
        ? `${primaryGuests.toLocaleString()} guest${primaryGuests === 1 ? '' : 's'}`
        : 'default tiers';
      const summaryParts = [
        `Top ${visible.length} of ${comparisons.length} package options for ${guestLabel}.`,
      ];
      if (normalizedSelectedVenue && normalizedSelectedPackage) {
        summaryParts.push('Your current selection is highlighted.');
        if (pinnedSelection) {
          summaryParts.push('It has been pinned below the top results for reference.');
        }
      }
      if (hasPercentAdjustments && weekdayLabel) {
        summaryParts.push(`Weekend adjustments reflect ${weekdayLabel.toLowerCase()} events.`);
      }
      if (hasFlatAdjustments && weekdayLabel) {
