(() => {
  ...
  async function exportPdf() {
    try {
      ...
      doc.autoTable({
        ...
        alternateRowStyles: {
          fillColor: [246, 242, 255],
          fillOpacity: 0.82,
        },
        foot: [['', '', '', 'Total', data.totalLabel]],
        footStyles: {
          fillColor: [76, 29, 149],
          fillOpacity: 0.9,
    ...
    try {
      ...
      const headerRow = new docx.TableRow({
        ...
        children: ['Item', 'Details', 'Qty', 'Unit', 'Total'].map((text) =>
          new docx.TableCell({
            ...
            shading: applyShadeType({ fill: 'EDE9FE', color: 'FFFFFF' }, 'PERCENT_20'),
      ...
      const bodyRows = data.rows.map((row, index) => {
        const zebraShadeFactory = () => (index % 2 === 0
          ? applyShadeType({ fill: 'FFFFFF', color: 'FFFFFF' }, 'PERCENT_10')
          : applyShadeType({ fill: 'F6F0FF', color: 'FFFFFF' }, 'PERCENT_20'));
        ...
        children: [
          new docx.TableCell({
            ...
            children: [
              new docx.Paragraph({
                alignment: docx.AlignmentType.RIGHT,
                children: [new docx.TextRun({ text: 'Total', bold: true, color: 'FFFFFF' })],
              }),
            ],
            shading: applyShadeType({ fill: '5D28B8', color: 'FFFFFF' }, 'PERCENT_20'),
          ...
          new docx.TableCell({
            children: [
              new docx.Paragraph({
                alignment: docx.AlignmentType.RIGHT,
                children: [new docx.TextRun({ text: data.totalLabel, bold: true, color: 'FFFFFF' })],
              }),
            ],
            shading: applyShadeType({ fill: '5D28B8', color: 'FFFFFF' }, 'PERCENT_20'),
            margins: { top: 100, bottom: 100, left: 120, right: 120 },
      ...
      const table = new docx.Table({
        ...
        borders: {
          top: { style: docx.BorderStyle.SINGLE, size: 4, color: 'D7CEF9' },
          bottom: { style: docx.BorderStyle.SINGLE, size: 4, color: 'D7CEF9' },
          left: { style: docx.BorderStyle.SINGLE, size: 4, color: 'D7CEF9' },
          right: { style: docx.BorderStyle.SINGLE, size: 4, color: 'D7CEF9' },
          insideHorizontal: { style: docx.BorderStyle.SINGLE, size: 4, color: 'E4DDFB' },
          insideVertical: { style: docx.BorderStyle.SINGLE, size: 4, color: 'E4DDFB' },
        },
      });
  ...
  function svgTextToPngDataUrl(svgText) {
    ...
    return renderSvgBlobToPng(blob, true).catch((primaryError) => {
      console.warn('SVG to PNG conversion via object URL failed. Falling back to data URL.', primaryError);
      return renderSvgBlobToPng(blob, false).catch((fallbackError) => {
        const error = fallbackError instanceof Error
          ? fallbackError
          : primaryError instanceof Error
            ? primaryError
            : new Error('Unable to convert SVG background to PNG.');
        throw error;
      });
    });
  }

  function renderSvgBlobToPng(blob, preferObjectUrl) {
    return new Promise((resolve, reject) => {
      ...
      image.onload = () => {
        try {
          const width = Math.max(1, image.naturalWidth || image.width || 1240);
          const height = Math.max(1, image.naturalHeight || image.height || 1754);
  ...
  function formatCurrency(value) {
    ...
    const [integerPartRaw, decimalPart] = numeric.toFixed(2).split('.');
